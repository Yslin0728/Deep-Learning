...
class MyOwnNetwork(ClassificationNet):
    def __init__(self, activation=Relu(), num_layer=2,
                 input_size=3 * 32 * 32, hidden_size=[32,64,128,256], #128,256,512,1024
                 std=1e-3, num_classes=10, reg=0.01, **kwargs): #
        ...
        # Initialize weights for all layers and zero bias
        self.num_layer = num_layer
        a = hidden_size[num_layer - 2]
        self.params = {'W1': std * np.random.randn(input_size, a),   #np.random.randn(input_size, a)/ np.sqrt(input_size/2)
                      'b1': np.zeros(a)}
        
        for i in range(num_layer - 2):
            a = hidden_size[num_layer - 2 - i]
            b = hidden_size[num_layer - 3 - i]
            
            self.params['W' + str(i + 2)] = std * np.random.randn(a,b)
            self.params['b' + str(i + 2)] = np.zeros(b)

        self.params['W' + str(num_layer)] = std * np.random.randn(hidden_size[0],
                                                                  num_classes)
        self.params['b' + str(num_layer)] = np.zeros(num_classes)
    ...
    def backward(self, dy):
        ...
        # self.params['W2'] = std * np.random.randn(256,128)
        # self.params['b2'] = np.zeros(128)
        
        # self.params['W3'] = std * np.random.randn(128,64)
        # self.params['b3'] = np.zeros(64)
        
        # self.params['W4'] = std * np.random.randn(64,32)
        # self.params['b4'] = np.zeros(32)
        
        # self.params['W5'] = std * np.random.randn(32,num_classes)
        # self.params['b5'] = np.zeros(num_classes)
        
        #cache_activation = self.cache['activation' + str(self.num_layer)]
        
        #std * np.random.randn(input_size, a)-